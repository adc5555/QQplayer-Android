...
function initialPermutation(input: Uint8Array): [number, number] {
  const v0 = input[0]! | (input[1]! << 8) | (input[2]! << 16) | (input[3]! << 24);
  const v1 = input[4]! | (input[5]! << 8) | (input[6]! << 16) | (input[7]! << 24);
...
function cryptBlock(input: Uint8Array, key: number[][]): Uint8Array {
  let [s0, s1] = initialPermutation(input);
  for (let i = 0; i < 15; i += 1) {
    const previous = s1;
    s1 = (feistel(s1, key[i]!) ^ s0) >>> 0;
    s0 = previous;
  }
  s0 = (feistel(s1, key[15]!) ^ s0) >>> 0;
  return inversePermutation(s0, s1);
}

function buildKeySchedule(key: Uint8Array, mode: number): number[][] {
  const schedule = Array.from({ length: 16 }, () => new Array<number>(6).fill(0));
  ...
  let c = 0;
  for (let i = 0; i < permC.length; i += 1) {
    const b = permC[i]!;
    const bit = b < 32 ? (v0 >>> (31 - b)) & 1 : (v1 >>> (63 - b)) & 1;
    c |= bit << (31 - i);
  }
  let d = 0;
  for (let i = 0; i < permD.length; i += 1) {
    const b = permD[i]!;
    const bit = b < 32 ? (v0 >>> (31 - b)) & 1 : (v1 >>> (63 - b)) & 1;
    d |= bit << (31 - i);
  }
  ...
  for (let i = 0; i < 16; i += 1) {
    ...
    const target = mode === DECRYPT ? 15 - i : i;
    const targetSchedule = schedule[target]!;
    for (let j = 0; j < 6; j += 1) targetSchedule[j] = 0;
    for (let j = 0; j < 24; j += 1) {
      const bit = (c >>> (31 - compression[j]!)) & 1;
      targetSchedule[j >> 3]! |= bit << (7 - (j & 7));
    }
    for (let j = 24; j < 48; j += 1) {
      const bit = (d >>> (31 - (compression[j]! - 27))) & 1;
      targetSchedule[j >> 3]! |= bit << (7 - (j & 7));
    }
  }
  return schedule;
...
export function tripledesKeySetup(key: Uint8Array, mode: number): number[][][] {
  if (mode === ENCRYPT) {
    return [
      buildKeySchedule(key.subarray(0, 8), ENCRYPT),
      buildKeySchedule(key.subarray(8, 16), DECRYPT),
      buildKeySchedule(key.subarray(16, 24), ENCRYPT)
    ];
  }
  return [
    buildKeySchedule(key.subarray(16, 24), DECRYPT),
    buildKeySchedule(key.subarray(8, 16), ENCRYPT),
    buildKeySchedule(key.subarray(0, 8), DECRYPT)
  ];
}

export function tripledesCrypt(data: Uint8Array, key: number[][][]): Uint8Array {
  let block = new Uint8Array(data);
  for (let i = 0; i < 3; i += 1) block = cryptBlock(block, key[i]!);
  return block;
}
